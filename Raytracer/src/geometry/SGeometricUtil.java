...
public class SGeometricUtil {
  ...
  public static int isOnCerclePerimeter(double x_C, double y_C, double R, double x, double y) throws SRuntimeException
  {
    ...
    // Puisque la méthode de l'ellipse utilise le diamètre, il faudra multiplier le rayon par deux (D = 2R).
    return isOnEllipsePerimeter(x_C, y_C, 2*R, 2*R, x, y);
  }
   ...
   * @param y La coordonnée y à analyser.
   * @return <b>-1</b> si (x,y) est à l'intérieur, <b>0</b> si (x,y) est sur le périmètre et <b>1</b> si (x,y) est à l'extérieur du périmètre.
   * @throws IllegalArgumentException Si la longeur du grand diamètre w ou la longueur du petit diamètre h est négative.
   */
  public static int isOnEllipsePerimeter(double x_E, double y_E, double w, double h, double x, double y) throws IllegalArgumentException
  {
    if(w < 0)
      throw new IllegalArgumentException("Erreur SGeometricUtil 004 : La valeur de w = " + w + " est négative.");
    
    if(h < 0)
      throw new IllegalArgumentException("Erreur SGeometricUtil 005 : La valeur de h = " + h + " est négative.");
  ...
  /**
   * Méthode pour obtenir la position d'un point sur une sphère à partir de deux angles.
   * 
   * @param r_S La position de la sphère.
   * @param R Le rayon de la sphère.
   * @param phi L'angle dans le plan xy (entre 0 et 2Pi).
   * @param theta L'angle selon l'axe z (entre 0 et Pi).
   * @return La position sur la sphère.
   */
  public static SVector3d sphericalCoordinate(SVector3d r_S, double R, double phi, double theta)
  {
    double x = R*Math.cos(phi)*Math.sin(theta);
    double y = R*Math.sin(phi)*Math.sin(theta);
    double z = R*Math.cos(theta);
    
    SVector3d r = new SVector3d(x, y, z);
   ...
   */
  public static SVector3d outsideSphereNormal(SVector3d rS, double R, SVector3d v) throws IllegalArgumentException
  {
    if(R < 0)
      throw new IllegalArgumentException("Erreur SGeometricUtil 006 : La valeur de R = " + R + " est négative.");
    ...
    if(compareDistanceWithReference(rS_to_v.modulus(), R) != 0)
      throw new IllegalArgumentException("Erreur SGeometricUtil 007 : Le vecteur v = " + v + " n'est pas sur la sphère (rS = " + rS + ", R = " + R + ").");
