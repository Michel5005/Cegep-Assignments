...
public class SLog {
  ...
  /**
	 * La constante <b>DEFAULT_LOG_FILE_NAME</b> correspond au nom du fichier <u>avec extension</u> de log par défaut étant {@value}.
	 */
  private static final String DEFAULT_LOG_FILE_NAME = "default_log" + "." + LOG_EXTENSION; 
	
  /**
   * La constante <b>EMPTY_LOG_DIRECTORY</b> représente le nom de répertoire vide étant {@value}.
   */
  private static final String EMPTY_LOG_DIRECTORY = "";
      
  /**
   * La constante <b>DEFAULT_LOG_DIRECTORY</b> correspond au répertoire par défaut où sera écrit le fichier log étant {@value}.
   */
  private static final String DEFAULT_LOG_DIRECTORY = EMPTY_LOG_DIRECTORY;
	...
	/**
	 * La variable <b>log_file_name</b> correspond au nom du fichier <u>avec extension</u> utilisé pour écrire le log.
	 */
	private static String log_file_name = DEFAULT_LOG_FILE_NAME;   
	
	/**
	 * La variabke <b>log_directory</b> correspond au nom du répertoire où sera enregistré le fichier log.
	 */
	private static String log_directory = DEFAULT_LOG_DIRECTORY;
	...
	/**
	 * La variable <b>isFileLogOn<b> détermine si les messages de type log seront redirigés vers un fichier texte en écriture.
	 */
	private static boolean isFileLogOn = true;						        
	
	/**
	 * La variable <b>isJTextPaneLogOn</b> détermine si les message de type log seront redidirigés vers un objet graphique de type JTextPane.
	 */
	private static boolean isJTextPaneLogOn = true;
	...
	/**
	 * La variable <b>textPane</b> correspond à une référence vers un objet graphique JTextPane où seront redirigé l'écriture du Log.
	 */
	 ...
	 * @param value
	 */
	public static void setJTextPaneLog(boolean value)
	{
	  isJTextPaneLogOn = value;
	}
	...
	/**
	 * Méthode pour initialiser le nom du fichier log (avec l'extension).
	 * 
	...
	public static void setLogFileName(String file_name) throws SRuntimeException, IOException
	{
	  ...
	  //Vérification que le fichier porte un extension admissible (.txt)		
		...
		// Définir le nom du fichier avec répertoire.
		String full_file_name;
		
		if(log_directory.equals(EMPTY_LOG_DIRECTORY))
		  full_file_name = log_file_name;
		else
		  full_file_name = log_directory + SStringUtil.FILE_SEPARATOR_CARACTER + log_file_name;
		
		try{
			fw = new FileWriter(full_file_name);
	...
	/**
   * Méthode pour initialiser le nom du répertoire du fichier log.
   * 
   * @param file_name Nom du fichier de sortir en format texte (de type .txt).
   * @throws SRuntimeException Si le nom de l'extension n'est pas de type approprié.
   * @throws IOException Si une erreur I/O s'est produit.
   */
  public static void setLogDirectory(String directory) throws SRuntimeException, IOException
  {
    log_directory = directory;
    
    // Relancer la création d'un fichier de log avec un nouvel emplacement.
    setLogFileName(log_file_name);
  }
  
	/**
	 * Méthode pour définir le JTextPane qui sera utilisé pour rediriger les messages Log.
	 * 
	 * @param textPane Le JTextPane où sera redirigé l'écriture.
	 */
	public static void setJTextPane(JTextPane pane)
	{
	  textPane = pane;
	  
	  isJTextPaneLogOn = true;
	}
   ...
   */
	 ...
	 */
	public static void logWriteLine(String str) throws SRuntimeException
	{
  ...
  public static void logWrite(String str)
  {
    ...
    // Écriture de un JTextPane.
    if(isJTextPaneLogOn)
      if(textPane != null)
      {
        // Référence : http://www.jguru.com/faq/view.jsp?EID=214352
        ...
        try {
          ...
          throw new SRuntimeException("Erreur SLog : Bad Location.", e);
        }
      }
  }
