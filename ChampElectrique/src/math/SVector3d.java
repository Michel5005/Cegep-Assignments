...
/**
 * <p>
 * La classe <b>SVector3d</b> repr�sente une vecteur <i>x</i>, <i>y</i> et
 * <i>z</i> � trois dimensions. Ce vecteur est immutable (<i>unmutable</i>).
 * </p>
 * 
 * <p>
...
public class SVector3d implements SVector, Comparable<SVector3d> {
	 ...
	 * variables x,y et z du vecteur �tant �gale � {@value}.
	 */
	private static final double DEFAULT_COMPONENT = 0.0;

	/**
	 * La constant <b>ZERO</b> correspond au vecteur nul correspondant � la
	 * coordonn�e (0.0, 0.0, 0.0).
	 */
	public static final SVector3d ZERO = new SVector3d(0.0, 0.0, 0.0);

	/**
	 * La constant <b>UNITARY_X</b> correspond au vecteur unitaire parall�le � l'axe
	 * x correspondant � (1.0, 0.0, 0.0). On utilise �galement la notation
	 * <b><i>i</i></b> pour le repr�senter.
	 */
	public static final SVector3d UNITARY_X = new SVector3d(1.0, 0.0, 0.0);

	/**
	 * La constant <b>UNITARY_Y</b> correspond au vecteur unitaire parall�le � l'axe
	 * y correspondant � (0.0, 1.0, 0.0). On utilise �galement la notation
	 * <b><i>j</i></b> pour le repr�senter.
	 */
	public static final SVector3d UNITARY_Y = new SVector3d(0.0, 1.0, 0.0);

	/**
	 * La constant <b>UNITARY_Z</b> correspond au vecteur unitaire parall�le � l'axe
	 * z correspondant � (0.0, 0.0, 1.0). On utilise �galement la notation
	 * <b><i>k</i></b> pour le repr�senter.
	 */
	public static final SVector3d UNITARY_Z = new SVector3d(0.0, 0.0, 1.0);

	/**
	 * La constante <b>ORIGIN</b> repr�sente un vecteur origine �tant situ� � la
	 * coordonn�e (0.0, 0.0, 0.0). Il est �gal au vecteur <b>ZERO</b>.
	 */
	...
	/**
	 * Constructeur d'un vecteur 3d en utilisant un string d�crivant les param�tres
	 * x, y et z du vecteur. Une lecture autoris�e peut prendre la forme suivante :
	 * "double x" "double y" "double z" (incluant la notation avec , ( ) [ ] comme
	 * d�limieur dans l'expression du string comme par exemple (2.3, 4.3, 5.7) ).
	 * 
	 * @param string - Le string de l'expression du vecteur en param�tres x, y, et
	 *               z.
	 * @throws SConstructorException Si le format de la chaine de caract�re est
	 *                               inad�quat.
	 */
	public SVector3d(String string) throws SConstructorException {
		try {
			...
			throw new SConstructorException(
					"Erreur SVector3d 001 : Une erreur est survenue lors de la construction du vecteur", e);
		}
	}
	...
	/**
	 * M�thode pour v�rifier si un vecteur est unitaire (de longueur �gal � 1).
	 * 
	 * @return <b>true</b>si le vecteur est unitaire et <b>false</b> sinon.
	 */
	...
	/**
	 * M�thode pour v�rifier si le vecteur est <u>exactement �gale � z�ro</u>.
	 * 
	 * @return <b>true</b>
	 */
	...
	/**
	 * M�thode pour g�n�rer un tableau des valeurs du vecteur. Il est important de
	 * pr�ciser que ce tableau est une copie des donn�es du vecteur. Modifier le
	 * tableau n'infuencera par le vecteur, car il est immutable (unmutable).
	 * 
	 * @return le vecteur sous le format d'un tableau.
	 */
	...
	/**
	 * M�thode pour <b>normaliser</b> un vecteur � trois dimensions. Un vecteur
	 * normalis� poss�de la m�me orientation, mais poss�de une <b> longeur unitaire
	 * </b>. Si le <b>module du vecteur est nul</b>, le vecteur normalis� sera le
	 * <b> vecteur nul </b> (0.0, 0.0, 0.0).
	 * 
	 * @return Le vecteur normalis�.
	 * @throws SImpossibleNormalizationException Si le vecteur ne peut pas �tre
	 *                                           normalis� �tant trop petit
	...
	public SVector3d normalize() throws SImpossibleNormalizationException {
		...
		// V�rification du module. S'il est trop petit, nous ne pouvons pas
		// num�riquement normaliser ce vecteur
	...
	public double dot(SVector3d v) {
		...
		produit = getX() * v.getX() + getY() * v.getY() + getZ() * v.getZ();
	...
	public SVector3d cross(SVector3d v) {
		...
		x = getY() * v.getZ() - getZ() * v.getY();
		y = getX() * v.getZ() - getZ() * v.getX();
		z = getX() * v.getY() - getY() * v.getX();
		
		produit = new SVector3d(x, -y, z);
	...
	/**
	 * M�thode pour obtenir un vecteur avec les coordonn�e (x,y,z) les plus petites
	 * (en consid�rant le signe) parmi un ensemble de vecteurs.
	 * 
	 * @param value_list La liste des vecteurs � analyser.
	 * @return Un vecteur ayant comme coordonn�e les plus petites valeurs (x,y,z)
	 *         trouv�es.
	 */
	...
	/**
	 * M�thode pour obtenir un vecteur avec les coordonn�e (x,y,z) les plus grandes
	 * (en consid�rant le signe) parmi un ensemble de vecteurs.
	 * 
	 * @param value_list La liste des vecteurs � analyser.
	 * @return Un vecteur ayant comme coordonn�e les plus grandes valeurs (x,y,z)
	 *         trouv�es.
	 */
	...
	/**
	 * M�thode pour �crire les param�tres xyz du vecteur dans un fichier en format
	...
	// ----------------------------------------------------------------------------------------------
	// M�thodes pour calcul sp�cialis� (afin de r�duire l'allocation de m�moire lors
	// des calculs)
	// ----------------------------------------------------------------------------------------------
	...
	/**
	 * M�thode permettant d'obtenir l'angle entre deux vecteurs A et B. L'angle sera
	 * obtenu en radian.
	 * 
	 * @param A Le premier vecteur.
	 * @param B Le second vecteur.
	 * @return L'angle entre les deux vecteurs en radian.
	 */
	...
	/**
	 * M�thode pour effectuer la <b>soustraction entre deux produits scalaires</b>.
	 * Cette op�ration vectorielle est �quivalente � l'expression
	 * <ul>
	 * (A - B) . C = A . C - B . C
	 * </ul>
	 * 
	...
	/**
	 * M�thode qui effectue le <b>produit mixte</b> de trois vecteurs A, B et C.
	 * Cette op�ration vectorielle est �quivalente � l'expression
	 * <ul>
	 * (A x B) . C = (Ax*By*Cz + Bx*Cy*Az + Cx*Ay*Bz) - (Az*By*Cx + Bx*Cz*Ay +
	 * Cy*Ax*Bz)
	 * </ul>
	 * Il est � remarquer que si deux des trois vecteurs sont �gaux ou colin�aires,
	...
	/**
	 * M�thode qui effectue le <b>triple produit vectoriel</b> de trois vecteurs A,
	 * B et C. Cette op�ration vectorielle est �quivalente � l'expression
	 * <ul>
	 * (A x B) x C = (A . C) B - (B . C) A
	 * </ul>
	 * <p>
	 * Il est important de pr�ciser que les deux expressions
	 * <ul>
	 * (A x B) x C != A x (B x C)
	 * </ul>
	 * ne sont pas �gaux. L'ordre de priorit� des op�rations doit �tre bien d�fini.
	 * </p>
	 * 
	 ...
	 * <p>
	 * M�thode qui effectue le <b>triple produit vectoriel</b> de trois vecteurs A,
	 * B et C avec l'ordre de priorit�
	 * </p>
	 * <ul>
	 * D = A x (B x C)
	 * </ul>
	 * <p>
	 * o� D est le r�sultat du triple produit vectoriel. Cette op�ration vectorielle
	 * est �quivalente � l'expression
	 * </p>
	 * <ul>
	 * A x (B x C) = (A . C) B - (A . B) C
	 * </ul>
	 * <p>
	 * et il est important de pr�ciser que les deux expressions
	 * </p>
	 * <ul>
	 * A x (B x C) != (A x B) x C
	 * </ul>
	 * <p>
	 * ne sont pas �gaux puisque le <b>produit vectoriel n'est pas commutatif</b>.
	 * </p>
	 * 
	 * @param A - Le 1ier vecteur dans le double produit vectoriel.
	 * @param B - Le 2i�me vecteur dans le double produit vectoriel.
	...
	/**
	 * M�thode qui effectue une op�ration sp�cialis�e de multiplication par un
	 * scalaire et d'addition vectorielle �quivalente �
	 * <ul>
	 * V = a*B + C.
	 * </ul>
	 * 
	...
	// ------------------------
	/**
	 * M�thode utilisant un string comme param�tre pour d�finir les composantes x, y
	 * et z du vecteur. Une lecture autoris�e peut prendre la forme suivante :
	 * "double x" "double y" "double z" (en incluant la notation avec , ( ) [ ] < >
	 * comme d�limieur dans l'expression du string comme par exemple (2.3, 4.3, 5.7)
	 * ).
	 * 
	 * @param string - Le string de l'expression du vecteur en param�tres x, y, et
	 *               z.
	 * @return un tableau de trois �l�ments tel que x = [0], y = [1] et z = [2].
	 * @throws IllegalArgumentException Si la chaine de caract�re n'est pas dans le
	 *                                  format ad�aquat.
	 */
	private double[] read(String string) throws IllegalArgumentException {
		StringTokenizer tokens = new StringTokenizer(string, SStringUtil.REMOVE_CARACTER_TOKENIZER);

		if (tokens.countTokens() != 3)
			throw new IllegalArgumentException("Erreur SVector3d 003 : L'expression '" + string
					+ "' ne contient pas exactement 3 param�tres pour les composantes xyz du vecteur 3d.");
		else {
			String s = ""; // String � convertir en double pour les composantes x, y et z.
			String comp = ""; // Nom de la composante en lecture utilis�e pour l'envoie d'une Exception s'il y
								// a lieu.
			...
			try {
				...
				throw new IllegalArgumentException("Erreur SVector3d 004 : L'expression '" + s
						+ "' n'est pas valide pour d�finir la composante '" + comp + "' du vecteur en cours.");
			}
		}
	}
