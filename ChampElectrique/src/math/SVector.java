...
public interface SVector extends SWriteable {
  ...
  {
    //-------------------------------------------------------------------------------------------------
    // Version Laboratoire : Le ray tracer.
    // throw new SNoImplementationException();
    //-------------------------------------------------------------------------------------------------
    
    //Calcul de l'interpolation : v = v0*(1 - t) + v1*t
    return v0.multiply(1.0 - t).add(v1.multiply(t));
  }
  ...
  public static SVector linearBarycentricInterpolation(SVector v0, SVector v1, SVector v2, double t1, double t2)
  {
    ...
    //Calcul de l'interpolation : v = v0*(1-t1-t2) + v1*t1 + v2*t2
    return (v0.multiply(1.0 - t1 - t2)).add(v1.multiply(t1)).add(v2.multiply(t2));
  }
  ...
  public static SVector linearBarycentricInterpolation(List<SVector> vector_list, List<Double> t_list) throws SRuntimeException
  {
    ...
    //Calcul de l'interpolation : v = v0*(1-t1-t2-...-tn) + v1*t1 + v2*t2 + ... + vn*tn
    SVector result = vector_list.get(0).multiply(1.0 - total); //le 1ier terme
    
    if(vector_list.size() == 1)    //s'il y a seulement 1 vecteur dans la liste
      return result;
    else
      for(int i = 0; i < t_list.size(); i++)
        result = result.add(vector_list.get(i+1).multiply(t_list.get(i).doubleValue()));
